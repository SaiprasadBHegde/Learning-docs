...
class EnrollmentService:
    ...
    async def enroll_student(
        ...
        if not course:
            raise AppError("Course not found", status_code=404)
        ...
        if existing:
            raise AppError("Student already enrolled in this course")

        # 3. Validate business rules
        await self._validate_enrollment(student, course)
        ...
        # 5. Update course count
        await self.course_repo.increment_enrollment_count(course_id)

        # 6. Send notification
        await self._send_enrollment_confirmation(student, course)
    ...
    async def drop_course(
        ...
        enrollment = await self.enrollment_repo.find_by_id(enrollment_id)

        if not enrollment:
            raise AppError("Enrollment not found", status_code=404)

        # Authorization
        if user_role == "student" and enrollment.student_id != user_id:
            raise AppError(
                "Cannot drop another student's course",
                status_code=403
            )

        # Check deadline
        if not self._is_before_drop_deadline(enrollment.semester):
            raise AppError("Drop deadline has passed")
        ...
        # Decrement count
        await self.course_repo.decrement_enrollment_count(
    ...
    async def _validate_enrollment(self, student, course):
        """Validate all enrollment business rules."""
        # Check prerequisites
        has_prereqs = await self._check_prerequisites(student, course)
        if not has_prereqs:
            prereq_names = await self._get_prerequisite_course_names(
                course.prerequisites
            )
            raise AppError(
                f"Missing prerequisites: {', '.join(prereq_names)}"
            )

        # Check capacity
        enrolled_count = await self.enrollment_repo.count_by_course(
            course.id
        )

        if enrolled_count >= course.max_capacity:
            raise AppError("Course is at maximum capacity")

        # Check credit limit
        exceeds_limit = await self._check_credit_limit(
            student.id,
            course.credits
        )
        ...
        if student.status != "active":
            raise AppError(
                f"Cannot enroll: student status is {student.status}"
            )

    async def _check_prerequisites(self, student, course) -> bool:
        """Check if student has completed prerequisites."""
        if not course.prerequisites:
            return True

        completed = await self.enrollment_repo.get_completed_courses(
            student.id
        )
    ...
    async def _check_credit_limit(
        ...
        current_credits = await self.enrollment_repo\
            .get_current_semester_credits(student_id, semester)

        return (current_credits + new_credits) > MAX_CREDITS
    ...
    def _is_before_drop_deadline(self, semester: str) -> bool:
        """Check if before drop deadline."""
        deadlines = {
            "Spring 2025": datetime(2025, 3, 1),
            "Summer 2025": datetime(2025, 7, 1),
            "Fall 2025": datetime(2025, 11, 1)
        }

        deadline = deadlines.get(semester)
        if not deadline:
            return False

        return datetime.now() < deadline

    async def _send_enrollment_confirmation(self, student, course):
        """Send confirmation email."""
        print(
            f"Email sent to {student.email}: "
            f"You are enrolled in {course.code} - {course.name}"
        )
```
...
async def enroll_student(self, student_id: str, course_id: str):
    async with self.db.begin():  # Transaction context
        ...
        await self.course_repo.increment_count(course_id)
